...
* Improved handling of numeric strings
* Better support for `BcMath\Number`
* Improved PHPDoc parsing
* More accurate generic template analysis
* Better handling of ambiguous array and foreach types

Attribute completion is also now context-aware.

Only valid PHP attribute classes are suggested — specifically classes marked with `#[Attribute]`.
...
* `is_null()` transformations
* Function override fixes
* Refactoring hints
...
* Fixed false warnings in HEREDOC strings
* Improved `__construct` reference tracking
* Better handling of `$GLOBALS` usage
* Improved PHPDoc nested tag parsing
* More accurate `@global` handling
* Better support for WordPress type conventions
* Improved generic types passed to base classes
* Optimized workspace loading and memory usage
* Faster re-analysis of edited files
* Improved Blade syntax handling
* Better namespace display in the Outline view
* Reduced unnecessary diagnostics for common patterns such as dummy `$_` variables
* Better compatibility with Alpine Linux (`musl`) installations
